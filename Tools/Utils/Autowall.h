...
public:
	...
	static float Autowall::GetDamagez(const Vector& point)
	{
		float damage = 0.f;
		Vector dst = point;
		FireBulletData data = FireBulletData(point);
		...
		g_Math.AngleVectors123(angles, &data.direction);
		Vector dataNormalized;

		data.direction.Normalize();


		if (FireSimulatedBullet(data, Hacks.LocalPlayer, Hacks.LocalWeapon))
		{
	...
	static float GetDamageResolver(Vector Target, Vector EyePos)
	{
		FireBulletData Bullet_Data = FireBulletData(EyePos);
