...
public:
	...
	/*static RECT GetTextSizee(DWORD font, const char* text)
	{
	...
	//static void Text123(int x, int y, CColor color, DWORD font, const wchar_t* text)
	//{
	//	Interfaces.pSurface->DrawSetTextFont(font);
	//	Interfaces.pSurface->DrawSetTextColor1(color);
	...
	static void Clear(int x, int y, int w, int h, CColor color)
	{
		Interfaces.pSurface->DrawSetColor1(color);
	...
	static void SpecList()
	{
		...
		for (int i = 0; i < Interfaces.pEntList->GetHighestEntityIndex(); i++)
		{
			...
			player_info_t pinfo;
			CColor forecolor = CColor(255, 165, 0, 255);
			// The entity isn't some laggy peice of shit or something

			if (!Vars.Visuals.Other.SpectatorList)
				return;
					...
					if (obs)
					{
						...
						if (pTarget)
						{
							if (Interfaces.pEngine->GetPlayerInfo(pTarget->GetIndex(), &pinfo2))
							{
								...
								RECT TextSize = GetTextSizee(Hacks.Font_ESP, buf);

								Clear(scrn.right - 300, 350 + (16 * x), 260, 16, CColor(39, 39, 39, 255));

								Interfaces.pSurface->DrawT((scrn.right - 300 + 130) - (TextSize.right / 2),
									350 + (16 * x), pTarget->GetIndex() == Hacks.LocalPlayer->GetIndex() ? forecolor : CColor(0, 0, 0, 0), Hacks.Font_Spectators,
									false, buf);
		...
		Clear(scrn.right - 308, 350 - 18, 276, 18, CColor(28, 28, 28, 255)); //top
		RECT TitleSize = GetTextSizee(Hacks.Font_Spectators, "Spectators");;

		Interfaces.pSurface->DrawT(scrn.right - 308 + 5,
			350 - 10 - (TitleSize.bottom / 2),
			CColor(255, 255, 255, 255),
			Hacks.Font_Spectators, false,
			"Spectators");

		Clear(scrn.right - 308, 350 + (16 * x), 276, 8, CColor(28, 28, 28, 255)); //bottom
		Clear(scrn.right - 308, 350, 8, (16 * x), CColor(28, 28, 28, 255)); //left
		Clear(scrn.right - 316 + 276, 350, 8, (16 * x), CColor(28, 28, 28, 255)); //right
	}*/
	...
	static void hitmarker()
	{
		...
		DrawLine(screenCenterX - lineSize * 2, screenCenterY + lineSize * 2, screenCenterX - (2), screenCenterY + (2), col);
		DrawLine(screenCenterX + lineSize * 2, screenCenterY + lineSize * 2, screenCenterX + (3), screenCenterY + (3), col);
		DrawLine(screenCenterX + lineSize * 2, screenCenterY - lineSize * 2, screenCenterX + (3), screenCenterY - (3), col);
	}
	...
	static void ThirdPerson()
	{
		...
		//if (enabledtp && Hacks.LocalPlayer->isAlive())
		//{
		//	*(bool*)((DWORD)Interfaces.pInput + 0xA5) = true;
		//	*(float*)((DWORD)Interfaces.pInput + (0xA8 + 0x8) + 0x8) = 100; // Thirdperson Distance
		//}
		//else
		//{
		//	*(bool*)((DWORD)Interfaces.pInput + 0xA5 + 0x8) = false;
		//	*(float*)((DWORD)Interfaces.pInput + (0xA8 + 0x8) + 0x8);
		//}
	}

	static auto skeet_border(int x, int y, int x2, int y2) -> void
	{
		Interfaces.pSurface->DrawSetColor(5, 5, 5, 255);
		Interfaces.pSurface->DrawFilledRect(x, y, x2, y2);

		Interfaces.pSurface->DrawSetColor(60, 60, 60, 255);
		Interfaces.pSurface->DrawFilledRect(x + 1, y + 1, x2 - 1, y2 - 1);

		Interfaces.pSurface->DrawSetColor(40, 40, 40, 255);
		Interfaces.pSurface->DrawFilledRect(x + 2, y + 2, x2 - 2, y2 - 2);

		Interfaces.pSurface->DrawSetColor(60, 60, 60, 255);
		Interfaces.pSurface->DrawFilledRect(x + 5, y + 3, x2 - 4, y2 - 3);

		Interfaces.pSurface->DrawSetColor(15, 15, 15, 255);
		Interfaces.pSurface->DrawFilledRect(x + 6, y + 4, x2 - 6, y2 - 4);
	}
	...
	static void CapsuleOverlay(int index, float duration)
	{
		if (!Vars.Visuals.Player.LastHittedhitbox)
			return;

		if (index == Interfaces.pEngine->GetLocalPlayer())
			return;

		auto pPlayer = reinterpret_cast <CBaseEntity*> (Interfaces.pEntList->GetClientEntity(index));
		if (!pPlayer)
			return;

		studiohdr_t* pStudioModel = Interfaces.g_pModelInfo->GetStudioModel(pPlayer->GetModel());
		if (!pStudioModel)
			return;

		static matrix3x4 pBoneToWorldOut[128];
		if (!pPlayer->SetupBones(pBoneToWorldOut, 128, 256, Interfaces.pGlobalVars->curtime))
			return;
		...
		for (int i = 0; i < pHitboxSet->numhitboxes; i++)
		{
			...
			if (pHitbox->radius > -1)
			{
				Interfaces.g_pDebugOverlay->AddCapsuleOverlay(vMin, vMax, pHitbox->radius, 0, 255, 255, 100, duration);
			}
		}
	}
	...
	static void DrawInfo( RECT rect, CBaseEntity* pPlayer )
	{
		...
		if( pPlayer->IsHostage() )
		{
			if( Vars.Visuals.Player.Hostage)
				texts.push_back( Text( "Hostage", 2, Hacks.Font_ESP, CColor(238, 255, 2, dormantAlpha[pPlayer->GetIndex()]) ) );
		}
		else if( Interfaces.pEngine->GetPlayerInfo( pPlayer->GetIndex(), &info ) )
		{
			if( Vars.Visuals.Player.Name )
				texts.push_back( Text( pPlayer->GetName(), 0, Hacks.Font_ESP, CColor(Vars.Visuals.Colors.esp_name_color, dormantAlpha[pPlayer->GetIndex()])) );

			if ( Vars.Visuals.Player.Money ) {
				texts.push_back(Text("$" + to_string(pPlayer->money()), 1, Hacks.Font_Controls, CColor(10, 175, 0, dormantAlpha[pPlayer->GetIndex()])));
			}
				

			if ( Vars.Visuals.Player.Flashed ) {

				if (pPlayer->FlashDuration() > 0 ) {
					texts.push_back(Text("flashed", 1, Hacks.Font_Controls, CColor(175, 175, 0, dormantAlpha[pPlayer->GetIndex()])));
				}
			}

			if (Vars.Visuals.Player.Zoom) {

				if (pPlayer->m_bIsScoped()) {
					texts.push_back(Text("zoom", 1, Hacks.Font_Controls, CColor(84, 155, 242, dormantAlpha[pPlayer->GetIndex()])));
				}
			...
			if( weapon )
			{
				if(Vars.Visuals.Player.Weapon && weapon )
					texts.push_back( Text( weapon->GetGunName(), 2, Hacks.Font_Controls, CColor(Vars.Visuals.Colors.esp_weapon_color, dormantAlpha[pPlayer->GetIndex()])) );
				if( Vars.Visuals.Player.Armor )
					texts.push_back( Text( pPlayer->GetArmorIcon(), 1, Hacks.Font_Controls, CColor(255, 255, 255, dormantAlpha[pPlayer->GetIndex()])) );
			}
			//delete[]weapon;
		}
		else if( pPlayer->IsWeapon() )
		{
			...
			if( weapon )
				texts.push_back( Text( ( char* )weapon->GetGunName(), 0, Hacks.Font_ESP, CColor(255, 255, 255, dormantAlpha[pPlayer->GetIndex()])) );
			int iClip = weapon->ammo();
			string s = iClip >= 0 && iClip <= 200 && weapon->IsGun() ? std::to_string(iClip) : "";
			if( weapon )
				texts.push_back( Text( strdup( ( s + "/" + std::to_string ( weapon->ammo2 () ) ).c_str() ), 2, Hacks.Font_ESP, CColor(255, 255, 255, dormantAlpha[pPlayer->GetIndex()])) );
			//delete[]weapon;
		}
		...
		for( auto text : texts )
		{
			...
			switch( text.side )
			{
				case 0:
					...
					Interfaces.pSurface->DrawT( middle - ( nameSize.right / 2 ), Top[ 0 ], text.color , text.Font, false, ( char* )text.text.c_str() );
					break;
				case 1:
					Interfaces.pSurface->DrawT( rect.right + 2, Top[ 1 ], text.color, text.Font, false, ( char* )text.text.c_str() );
					Top[ 1 ] += nameSize.bottom + 1;
					break;
				case 2:
					Interfaces.pSurface->DrawT( middle - ( nameSize.right / 2 ), Top[ 2 ], text.color, text.Font, false, ( char* )text.text.c_str() );
			...
			if (Interfaces.pEngine->GetPlayerInfo(pEntity->GetIndex(), &info))
			{
				...
				if (pEntity->IsDormant() && dormantAlpha[pEntity->GetIndex()] != 0)
				{
					if (dormantAlpha[pEntity->GetIndex()] < 0.f)
						...
						dormantAlpha[pEntity->GetIndex()] -= 1.f;
				}
	...
	static void DrawFovArrows(CBaseEntity* entity)
	{
		...
		Vector screenPos;
		if (IsOnScreen(get_hitbox_pos(entity, (int)CSGOHitboxID::HITBOX_PELVIS), screenPos)) return;
		...
		const auto rot = DEG2RADD(client_viewangles.y - g_Math.CalcAngle(local_position, get_hitbox_pos(entity, (int)CSGOHitboxID::HITBOX_PELVIS)).y - 90);
	...
	static void DrawHealth( RECT rect, CBaseEntity* pPlayer )
	{
		...
		DrawRect( rect.left - 4, rect.top - 1, rect.left - 2, rect.bottom + 1, CColor( 0, 0, 0, dormantAlpha[pPlayer->GetIndex()] / 2) );
