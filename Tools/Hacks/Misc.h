...
public:
			...
			else
			{
				if (!(Hacks.LocalPlayer->GetFlags() & FL_ONGROUND))
				{
					...
					else if (GetAsyncKeyState(0X41)) //a
					{
						...
						Hacks.CurrentCmd->sidemove = (-1550.f * 5) / Hacks.LocalPlayer->GetVecVelocity().Length2D();
						Hacks.CurrentCmd->forwardmove = (Hacks.CurrentCmd->command_number % 2) == 0 ? 450.f : -450.f;
						if (Hacks.CurrentCmd->sidemove < -450.f)
							Hacks.CurrentCmd->sidemove = -450.f;
					}
	...
	static void Resolver()
	{
		if (Hacks.LocalPlayer && Hacks.LocalPlayer->isAlive())
		{
			for (auto i = 0; i < 65; ++i)
			{
				...
				//if (entity->GetEyeAnglesPointer()->x >= 0)
				//	entity->GetEyeAnglesPointer()->x = 89;

				//static float LastYaw[65];

				//LastYaw[entity->GetIndex()] = entity->GetEyeAnglesPointer()->y;

				//if (entity->GetActivity(entity->GetAnimOverlay(4)->m_nSequence) == 979
				//	/*|| entity->GetActivity(entity->GetAnimOverlay(2)->m_nSequence) == 972*/)
				//{
				//	if (entity->GetAnimOverlay(4)->m_flWeight <= 0.05
				//		|| entity->GetAnimOverlay(2)->m_flWeight <= 0.05)
				//	{
				//		entity->GetEyeAnglesPointer()->y = LastYaw[entity->GetIndex()];
				//	}
				//}
			}
		}
	...
	static void SkipAnimation(CBaseEntity *player)
	{
		static ConVar *sv_pvsskipanimation = Interfaces.g_ICVars->FindVar("sv_pvsskipanimation");
		...
		*(int32_t*)((uintptr_t)player + 0xEC) |= 8;
		player->SetupBones(NULL, -1, 0x7FF00, Interfaces.pGlobalVars->curtime);
