...
void inline SinCosNew(float radians, float *sine, float *cosine)
{
	*sine = sin(radians);
	*cosine = cos(radians);
}

void CMath::AngleVectors123(const Vector &angles, Vector *forward)
{
	float sp, sy, cp, cy;

	SinCosNew(DEG2RAD(angles[1]), &sy, &cy);
	SinCosNew(DEG2RAD(angles[0]), &sp, &cp);
